...
const FacultyDashboard = () => {
  ...
  useEffect(() => {
    if (facultyId) {
      ...
      axiosInstance.get(`/bookings?facultyId=${facultyId}`)
  ...
  const capitalizeCategory = (cat) => cat.charAt(0).toUpperCase() + cat.slice(1).toLowerCase();
            ...
            <TabsContent value="venue">
              <Card className="mt-6 animate-fade-in">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="mt-8">
                    <h3 className="text-lg font-semibold mb-2">My Booking Requests</h3>
                    <div className="overflow-x-auto">
                          ...
                          {myBookings.length > 0 ? myBookings.map(b => (
                            <tr key={b._id}>
                              ...
                              <td className="px-2 py-1">
                                <span className={`px-2 py-1 rounded text-xs ${b.status === 'approved' ? 'bg-green-200 text-green-800' : b.status === 'pending' ? 'bg-yellow-200 text-yellow-800' : 'bg-red-200 text-red-800'}`}>{b.status.charAt(0).toUpperCase() + b.status.slice(1)}</span>
                              </td>
                            </tr>
                          )) : (
                            <tr><td colSpan={5} className="text-center py-4 text-gray-500">No booking requests found.</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
